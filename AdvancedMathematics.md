...
### §2 渐近线判断
1. 水平渐近线： 
2.  $$\lim_{x \to \infty}f(x)=A$$,y=A为水平渐近线
3. 铅直渐近线：  
4. $$f(a-0)=\infty 或 f(a+0)=\infty 或 \lim_{x \to a}f(x)=\infty$$，x=a为铅直渐近线
5. 斜渐近线：
6. $$\lim_{x \to \infty}\frac{f(x)}{x}=a,\lim_{x \to \infty}[f(x)-ax]=b$$,则y=ax+b为斜渐近线
**注意**：  
（1）铅直渐近线一定存在间断点  
